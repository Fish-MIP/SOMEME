...
```{r}
#CCAMLR boundaries
ccamlr <- read_sf("../../SO_shapefiles/CCAMLR_Convention_Area/statistical_areasPolygon_wgs84.shp")
...
ccamlr_ras <- rasterize(ccamlr, fishmip, field = "GAR_Short_", touches = T,
                        fun = "min", background = NA)
#Saving raster
writeCDF(ccamlr_ras, file.path(out_folder, "ccamlr_stats_area_1deg.nc"),
         overwrite = T, varname = "ccamlr", longname = "ccamlr stats areas",
         missval = NA)

#Get keys to interpret mask
ccamlr |> 
  st_drop_geometry() |> 
  select(GAR_Name, GAR_Short_) |> 
  write_csv(file.path(out_folder, "ccamlr_stats_area_keys.csv"))
...
ccamlr_df |> 
  write_csv(file.path(out_folder, "ccamlr_stats_area_1deg.csv"))
